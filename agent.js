...
class AIAgent {
    ...
    async init() {
        try {
            ...
            console.log(`AI Agent v${this.version} initialized successfully`);
        } catch (error) {
    ...
    setupEventListeners() {
        try {
            ...
            document.getElementById('cleanup-conversations')?.addEventListener('click', () => this.cleanUpConversations());
    ...
    async callLLM(conversation) {
        ...
        // Filter out system messages that are error messages to prevent corruption
        const messagesForApi = conversation.messages
            .filter(m => {
                // Skip system messages that contain error information
                if (m.role === 'system' && m.content && typeof m.content === 'string') {
                    return !m.content.includes('Agent iteration error') && 
                           !m.content.includes('API Error') && 
        ...
        // Basic API key validation - only check for obvious issues
        const cleanApiKey = apiKey.trim();
        if (cleanApiKey.length < 5) {
            throw new Error('API key appears to be too short. Please check your API key in settings.');
        }
        ...
        switch (provider) {
            ...
            case 'google':
                ...
                apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${cleanApiKey}`;
        ...
        try {
            ...
            if (!resp.ok) {
                ...
                try { 
                    ...
                    console.log('API Error Details:', errJson);
                } catch (_) {
                ...
                if (resp.status === 401) {
                    ...
                    throw new Error(`Model or endpoint not found. Please check your model selection.`);
                } else {
            ...
            if (err.name === 'TypeError' && err.message.includes('fetch')) {
                throw new Error('Network error: Cannot reach the API server. Please check your internet connection and try again.');
            } else if (err.name === 'AbortError') {
                throw new Error('Request timeout: The API request took too long. Please try again.');
            } else if (err.message.includes('CORS')) {
                throw new Error('CORS error: The API request was blocked. This might be a browser security issue.');
                ...
                throw new Error('Connection failed: Unable to reach the API server. Please check your internet connection.');
            }
            ...
            // Generic fallback
            throw new Error(`Request failed: ${err.message || 'Unknown error occurred'}`);
        }
    }
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'AI Agent', system: 'System', tool: 'Tool' }[message.role] || message.role;
        ...
        try {
            ...
            if (stored) {
                ...
                Array.from(this.state.conversations.values()).forEach(conv => { 
                    ...
                    // Clean up error messages from conversation history
                    conv.messages = conv.messages.filter(msg => {
                        if (msg.role === 'system' && msg.content && typeof msg.content === 'string') {
                            return !msg.content.includes('Agent iteration error') && 
                                   !msg.content.includes('API Error') && 
    ...
    updateSettingsFromForm() {
        ...
        const rawApiKey = document.getElementById('api-key')?.value || '';
        s.llm.apiKey = rawApiKey.trim();
    ...
    // Clean up corrupted conversation data
    cleanUpConversations() {
        try {
            Array.from(this.state.conversations.values()).forEach(conv => {
                // Remove error messages from conversation history
                conv.messages = conv.messages.filter(msg => {
                    if (msg.role === 'system' && msg.content && typeof msg.content === 'string') {
                        return !msg.content.includes('Agent iteration error') && 
                               !msg.content.includes('API Error') && 
            ...
            this.saveCurrentConversation();
            this.showToast('success', 'Cleanup Complete', 'Corrupted conversation data has been cleaned up.');
        } catch (e) {
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to AI Agent! How can I assist you?');
        }
    }
